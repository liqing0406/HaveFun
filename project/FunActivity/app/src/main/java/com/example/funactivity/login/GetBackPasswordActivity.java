...
public class GetBackPasswordActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        submit.setOnClickListener(v -> {
            ...
            intent.setClass(v.getContext(), GetBackPasswordNextActivity.class);
    ...
    private void alertWarning() {
        @SuppressLint("ShowToast") DialogInterface.OnClickListener listener = (dialog, which) -> {
            switch (which) {
                ...
                case Dialog.BUTTON_NEGATIVE:
                    dialog.dismiss();
                    Toast.makeText(GetBackPasswordActivity.this, "已取消", Toast.LENGTH_SHORT);
                    break;
            }
        };
        //dialog参数设置,方法链调用
    ...
    public void isRegister() {
        ...
        call.enqueue(new Callback() {
            ...
            public void onResponse(Call call, Response response) throws IOException {
                ...
                if (back.equals("false")) {//已经注册
                    Message message = new Message();
                    message.what = 2;
                    handler.sendMessage(message);
                } else if ("true".equals(back)) {//手机号未注册
                    Looper.prepare();
                    Toast.makeText(GetBackPasswordActivity.this, "此手机号码未注册", Toast.LENGTH_SHORT).show();
                    Looper.loop();
                }
            }
        });
